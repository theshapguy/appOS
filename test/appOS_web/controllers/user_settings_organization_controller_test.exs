defmodule AppOSWeb.UserSettingsOrganizationControllerTest do
  ...
  describe "PUT /user/settings/team (update user role)" do
    ...
    test "update the team member to admin", %{
      ...
      [%AppOS.Roles.Role{} = admin_role] = user.roles
      ...
      assert AppOS.Accounts.get_user!(another_user.id).organization_admin?
    end
    ...
    test "remove the team member from admin - if no other team admin fails", %{
      ...
      assert AppOS.Accounts.get_user!(user.id).organization_admin? == true
    end
    ...
    test "remove the team member from admin - if another admin exists in the team ", %{
      ...
      assert AppOS.Accounts.get_user!(user.id).organization_admin? == false
    end
  ...
  describe "PUT /user/settings/team (add organization member)" do
    ...
    setup %{user: user} do
      [admin_role, simple_role] = AppOS.Roles.list_roles(user.organization)
