defmodule Planet.Plug.StripeSignature do
  ...
  def verify_webhook_signature(conn) do
    ...
    with {:ok, signature_header} <- get_stripe_signature(headers),
  ...
  @spec get_stripe_signature(map()) :: {:ok, binary()} | {:error, :missing_signature}
  defp get_stripe_signature(headers) do
    case Map.fetch(headers, "stripe-signature") do
  ...
  defp parse_signature(signature_header) do
    ...
    timestamp_part = Enum.find(parts, fn part -> String.starts_with?(part, "t=") end)
    signature_parts = Enum.filter(parts, fn part -> String.starts_with?(part, "v1=") end)

    with "t=" <> timestamp <- timestamp_part,
         [_ | _] = signatures <- signature_parts do
      v1_signatures = Enum.map(signatures, fn "v1=" <> sig -> sig end)
  ...
  defp error_message(:missing_signature), do: "Stripe-Signature is missing from headers"
